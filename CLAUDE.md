...
- Crawler: `extract_article_title` убирает шапочный h1 с полным именем ЯрГУ; порядок og → контентные заголовки → `<title>`; `upsert_document` обновляет при конфликте URL без смены `html_hash`. Паук recall-first: один `parse`, `LinkExtractor` (deny как в yagu: upload/bitrix/медиа/файлы), старт с главной + листинги новостей/событий/прессы; почти все HTML-страницы сайта (включая листинги и `PAGEN_`), trafilatura + fallback текста из DOM; `-a only_new=true`, `-a max_depth=5`, `-a link_limit=200`.
